...
const findAllProperties = async (queryLimit, queryOffset) => {
...
const insertProperty = async (newProperty) => {
    ...
    try {
        ...
        const imovelResult = await client.query(`INSERT INTO ${TABLE_NAME_IMOVEIS} (${imoveisKeys.join(", ")}) VALUES (${imoveisParams}) RETURNING id`, imoveisValues);
        ...
        const sistemaResult = await client.query(`INSERT INTO ${TABLE_NAME_SISTEMA} (${sistemaKeys.join(", ")}) VALUES (${sistemaParams}) RETURNING id`, sistemaValues);
...
const insertImovelImages = async (imovelId, filePaths) => {
    const client = await db.connect();
...
export { findAllProperties, findPropertyById, updatePropertyById, insertProperty, deletePropertyById, insertImovelImages, };
