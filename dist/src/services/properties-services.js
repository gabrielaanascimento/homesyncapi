...
const getPropertiesService = async (queryLimit, queryOffset) => {
    // ... (lógica inalterada)
...
const createPropertyService = async (property) => {
    // ... (lógica inalterada)
    let response;
    if (Object.keys(property).length != 0) {
        try {
            const newSistemaId = await propertiesRepository.insertProperty(property);
            if (newSistemaId) {
...
const updatePropertyService = async (id, propertyData) => {
    // ... (lógica inalterada)
    let responseUpdate;
    try {
...
const insertImovelImagesService = async (sistemaImovelId, filePaths) => {
    try {
...
export { getPropertyByIdService, getPropertiesService, createPropertyService, deletePropertyService, updatePropertyService, insertImovelImagesService, // EXPORT NEW FUNCTION
 };
