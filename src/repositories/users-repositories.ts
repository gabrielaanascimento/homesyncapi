...
async function findAllUsers(): Promise<CorretorModel[]> {
  const result = await db.query(`SELECT ${CORRETOR_FIELDS} FROM ${TABLE_NAME}`);
  return result.rows as CorretorModel[];
}

const findUserById = async (id: number): Promise<CorretorModel | null> => {
    const result = await db.query(`SELECT ${CORRETOR_FIELDS} FROM ${TABLE_NAME} WHERE id = $1`, [id]);
    return result.rows[0] || null;
};

const updateUserById = async (id: number, data: Partial<CorretorModel>): Promise<boolean> => {
...
const insertUser = async (newUser: Omit<CorretorModel, "id">): Promise<number | null> => {
