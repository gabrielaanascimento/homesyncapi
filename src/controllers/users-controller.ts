...
export const postUser = async(req: Request, res: Response) => {
    const bodyValue: Omit<CorretorModel, "id"> = req.body; // CORRIGIDO
...
export const updateUser = async(req: Request, res: Response) => {
    const id = parseInt(req.params.id);
    const bodyValue: Partial<CorretorModel> = req.body; // CORRIGIDO
    const httpResponse = await service.updateUserService(id, bodyValue);
    
    res.status(httpResponse.statusCode).json(httpResponse.body);
}

export const uploadCorretorPhoto = async (req: AuthRequest, res: Response) => {
// ... (lógica inalterada)
    if (!req.file) {
